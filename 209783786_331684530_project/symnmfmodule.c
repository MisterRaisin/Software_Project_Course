...
static PyObject* py_sym(PyObject* self, PyObject* args) {
    ...
    double** similarity_matrix = compute_similarity_matrix(data, n, d);
    if (!similarity_matrix) {
        PyErr_SetString(PyExc_RuntimeError, "Failed to compute similarity matrix");
        return NULL;
    }

    PyObject* result = convert_matrix_to_pylist(similarity_matrix, n, n);
    free_matrix(data, n);
...
static PyObject* py_ddg(PyObject* self, PyObject* args) {
    ...
    double* degree_matrix = compute_degree_matrix(data, n, d);
    if (!degree_matrix) {
        PyErr_SetString(PyExc_RuntimeError, "Failed to compute degree matrix");
        return NULL;
    }

    PyObject* result = convert_vector_to_pylist(degree_matrix, n);
    free_matrix(data, n);
    free(degree_matrix);
...
static PyObject* py_norm(PyObject* self, PyObject* args) {
    ...
    double** norm_matrix = compute_normalized_similarity(data, n, d);
    if (!norm_matrix) {
        PyErr_SetString(PyExc_RuntimeError, "Failed to compute normalized similarity matrix");
        return NULL;
    }

    PyObject* result = convert_matrix_to_pylist(norm_matrix, n, n);
    free_matrix(data, n);
...
static PyObject* py_symnmf(PyObject* self, PyObject* args) {
    ...
    double** H_final = perform_symnmf(W, H_init, n, k);
    if (!H_final) {
        PyErr_SetString(PyExc_RuntimeError, "Failed to perform SymNMF");
        return NULL;
    }

    PyObject* result = convert_matrix_to_pylist(H_final, n, k);
    free_matrix(W, n);
